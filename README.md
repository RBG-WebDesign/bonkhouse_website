...
- **Homepage effects** live in `public/home-effects.js`. The hero and photo rotation pause when hidden or offscreen and respect reduced motion. Gallery photos load as needed.
- **Shared visual styles** live in `public/bonkhouse.css` (type, spacing, buttons, fields, and states) and `public/site-header.css`. Static pages link these stylesheets; React imports them in `app/layout.tsx`. Page-specific styles live in `public/home.css`, `public/event.css`, and `public/archive.css`. Keep the shared rules consistent across both page types. `public/support.js` is generated runtime code.
