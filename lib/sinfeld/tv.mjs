...
export async function startSinfeldTv() {
    ...
    return new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          vec2 curved = centered * (1.0 + 0.075 * radius2);
          ...
          float aberration = 0.0017 + radius2 * 0.0013;
  ...
  function render(time) {
    ...
    resizeRenderer();
    drawGameFeed(time);
    if (screenMaterial) screenMaterial.uniforms.time.value = time * 0.001;
    if (crtLight) {
    ...
    const distance = CAMERA_DISTANCE - punch * punch * 0.45;
