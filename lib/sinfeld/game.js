...
const canvas = document.querySelector("#game");
  ...
  async function loadGame() {
    try {
      ...
      titleClock = 0;
      ready = true;
      unlockGameAudio(false); // context stays suspended until a gesture, but the buffers decode now
  ...
  function startJump() {
    ...
    lastGruntIndex = playRandomSound(GRUNT_SFX, 1.7, lastGruntIndex, 0.52);
  }
  ...
  function setupGameAudioGraph() {
    ...
    gameAudioOutput.threshold.value = -7;
    gameAudioOutput.knee.value = 12;
    gameAudioOutput.ratio.value = 4;
    ...
    gameAudioOutput.connect(speaker).connect(rumbleCut).connect(cabinet).connect(trebleCut).connect(drive)
      .connect(gameAudioContext.destination);
    musicSource = gameAudioContext.createMediaElementSource(music);
    musicSource.connect(gameAudioOutput);
  ...
  async function startMusicFromGesture() {
    if (!music.paused || music.dataset.userPaused === "true") return;
    try {
      music.volume = activeVoice ? MUSIC_DUCKED_VOLUME : MUSIC_VOLUME;
      await music.play();
  ...
  function playSound(url, volume = 1, playbackRate = 1, reverbAmount = 0) {
    ...
    if (!buffer || gameAudioContext?.state !== "running") return null;
  ...
  function playVoice(url, interrupt = false) {
    ...
    const voice = playSound(url, 1.95, 1, 0.58);
    if (!voice) return false;
    activeVoice = voice;
    music.volume = MUSIC_DUCKED_VOLUME;
  ...
  function beginDeathSequence() {
    ...
    for (const car of cars) stopCarSound(car);
    stopActiveVoice();
    lastDeathIndex = playRandomSound(DEATH_SFX, 1.9, lastDeathIndex, 0.64);
  }
  ...
  function damagePlayer() {
    ...
    else lastGruntIndex = playRandomSound(GRUNT_SFX, 1.7, lastGruntIndex, 0.52); // survivable hit: grunt, not a scream
  }
  ...
  document.querySelector("#game").addEventListener("pointerdown", () => document.querySelector("#game").dispatchEvent(new CustomEvent("tv:click", { bubbles: true })));
